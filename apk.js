...
(() => {
  ...
  function allowlist() {
    var listed;
    try {
      if (typeof globalThis !== "undefined" && globalThis.__BANAN_APK_ALLOWLIST)
        listed = globalThis.__BANAN_APK_ALLOWLIST;
      else
        listed = typeof __BANAN_APK_ALLOWLIST !== "undefined" ? __BANAN_APK_ALLOWLIST : [];
    } catch (_) {
      listed = [];
    }
  ...
  function logLine(message) {
    try {
      const file = new File("/data/data/com.supercell.brawlstars/files/banan/agent.log", "a");
      file.write(message + "\n");
      file.close();
    } catch (_) {
    }
  ...
  function readIdentity() {
    const paths = [
      "/data/data/com.supercell.brawlstars/files/banan/identity.txt",
      "/data/user/0/com.supercell.brawlstars/files/banan/identity.txt"
    ];
  ...
  function readBananText(name) {
    const paths = [
      "/data/data/com.supercell.brawlstars/files/banan/" + name,
      "/data/user/0/com.supercell.brawlstars/files/banan/" + name
    ];
    for (let i = 0; i < paths.length; i++) {
      try {
        if (typeof File !== "undefined" && typeof File.readAllText === "function") {
  ...
  function watchFlags(toggleFeature2, isFeatureEnabled2) {
    ...
    setInterval(() => {
      try {
        const text = readBananText("flags.txt");
